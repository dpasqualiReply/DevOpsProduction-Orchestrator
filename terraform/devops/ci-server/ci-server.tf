...
resource "google_compute_instance" "ci-server" {
 ...
 provisioner "local-exec" {
  ...
  command = "sleep 90; ansible-playbook -i '${google_compute_instance.ci-server.name},' --private-key=~/.ssh/ansible_rsa /opt/ansible/ci-server.yml -e 'ansible_ssh_user=dario_pasquali93' -e 'host_key_checking=False'"
 }
}
